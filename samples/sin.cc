...
main(){
  ...
  win[0].open( 10, 10 )               // open the 1st window win[0]
    ...
    .EPSOn()                     // enable EPS output for win[0]
    .map();
  ...
  gxt0.setRatio( tmax*h, 1.0, gwidth, gheight )//The value tmx*h is gwidth on screen...
    .setAxisWithDigit(axisDigit)    //show axis digits
    .axis();
  ...
  gxy.setRatio( 2.0, 2.0, 150, 150 )
    ...
    .setFgColor("cyan")
    .setAxisWithDigit(axisDigit)
    .EPSOn()                           // start recording the graph
  ...
  win[0].EPSOut("test.eps");  // output the graph on win[0] by eps format
  gxt1[0].EPSOut("test2.eps");// output the graph gxt1[0] by eps format
  win[1].EPSOut("test3.eps"); // output the graph on win[1] by eps format
